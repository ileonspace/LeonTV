...
| 位置 | 需要修改 | 说明 |
|------|---------|------|
| `index.html` 登录密码 | `YOUR_PASSWORD` → 你的密码 | 搜索 `if(v==='YOUR_PASSWORD')` |
| `functions/api/[[path]].js` | `YOUR_PASSWORD` → 你的密码 | 第6行 `AUTH_PWD` |
| `tmdb-worker.js` | `PASSWORD` 环境变量 | Cloudflare Dashboard 设置 |
| TMDB 请求密码 | `?pwd=YOUR_PASSWORD` → 你的密码 | index.html 第~900行 |
