...
搜索代码中的 `YOUR_PASSWORD` 替换为你自己的密码（共4处）：

```bash
grep -rn "YOUR_PASSWORD" index.html functions/
```

| 位置 | 说明 |
|------|------|
| `index.html` 登录 | 前端页面访问密码 |
| `index.html` TMDB | Worker 通信密码（需与 Worker 一致） |
| `functions/api/[[path]].js` | API 接口认证 |
| `tmdb-worker.js` | 通过 **Cloudflare 环境变量** `PASSWORD` 设置（不写代码里） |

> 建议使用随机生成的复杂密码，不要使用常见词汇。
...
**Q: 如何修改默认密码？**
A: 搜索 `YOUR_PASSWORD` 在 `index.html` 中替换为你的密码。
